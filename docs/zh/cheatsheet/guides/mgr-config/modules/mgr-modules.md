...
| 选项 | 默认值 | 级别 | 调优 |
|--------|---------|-------|--------|
| [cephadm_path](#cephadm_path) | `/usr/sbin/cephadm` | Advanced | 容量 |
| [mgr_client_bytes](#mgr_client_bytes) | `128_M` | Dev | 开发 |
| [mgr_client_messages](#mgr_client_messages) | `512` | Dev | 开发 |
| [mgr_data](#mgr_data) | `/var/lib/ceph/mgr/$cluster-$id` | Advanced | 性能 |
| [mgr_debug_aggressive_pg_num_changes](#mgr_debug_aggressive_pg_num_changes) | `False` | Dev | 开发 |
| [mgr_disabled_modules](#mgr_disabled_modules) | `0` | Advanced | 性能 |
| [mgr_initial_modules](#mgr_initial_modules) | `iostat nfs nvmeof` | Basic | 策略 |
| [mgr_max_pg_creating](#mgr_max_pg_creating) | `1024` | Advanced | 性能 |
| [mgr_max_pg_num_change](#mgr_max_pg_num_change) | `128` | Advanced | 性能 |
| [mgr_mds_bytes](#mgr_mds_bytes) | `128_M` | Dev | 开发 |
| [mgr_mds_messages](#mgr_mds_messages) | `128` | Dev | 开发 |
| [mgr_module_load_delay](#mgr_module_load_delay) | `0` | Dev | 开发 |
| [mgr_module_load_delay_name](#mgr_module_load_delay_name) | `(empty)` | Dev | 开发 |
| [mgr_module_load_expiration](#mgr_module_load_expiration) | `20000` | Dev | 开发 |
| [mgr_module_monitor_interval](#mgr_module_monitor_interval) | `5` | Advanced | 性能 |
| [mgr_module_path](#mgr_module_path) | `0/mgr` | Advanced | 容量 |
| [mgr_mon_bytes](#mgr_mon_bytes) | `128_M` | Dev | 开发 |
| [mgr_mon_messages](#mgr_mon_messages) | `128` | Dev | 开发 |
| [mgr_osd_bytes](#mgr_osd_bytes) | `512_M` | Dev | 开发 |
| [mgr_osd_messages](#mgr_osd_messages) | `8_K` | Dev | 开发 |
| [mgr_osd_upgrade_check_convergence_factor](#mgr_osd_upgrade_check_convergence_factor) | `0.8` | Advanced | 性能 |
| [mgr_pool](#mgr_pool) | `True` | Dev | 开发 |
| [mgr_service_beacon_grace](#mgr_service_beacon_grace) | `1_min` | Advanced | 性能 |
| [mgr_standby_modules](#mgr_standby_modules) | `True` | Advanced | 性能 |
| [mgr_stats_period](#mgr_stats_period) | `5` | Basic | 策略 |
| [mgr_stats_period_autotune](#mgr_stats_period_autotune) | `True` | Basic | 策略 |
| [mgr_stats_period_autotune_queue_threshold](#mgr_stats_period_autotune_queue_threshold) | `100` | Advanced | 性能 |
| [mgr_stats_threshold](#mgr_stats_threshold) | `5` | Advanced | 性能 |
| [mgr_subinterpreter_modules](#mgr_subinterpreter_modules) | `0` | Advanced | 性能 |
| [mgr_test_metadata_error](#mgr_test_metadata_error) | `False` | Dev | 开发 |
| [mgr_tick_period](#mgr_tick_period) | `2` | Advanced | 性能 |
...
**作用：** List of manager modules to enable when the cluster is first started This list of module names is read by the monitor when the cluster is first started after installation, to populate the list of enabled manager modules. Subsequent updates are done using the 'mgr module &#91;enable\
...
**作用：** Number of milliseconds for Manager modules to delay loading. For testing purposes only. Number of milliseconds for Manager modules to delay loading. For testing purposes only.
...
**作用：** Specify which Manager module is to delay loading by mgr_module_load_delay milliseconds. For testing purposes only. Specify which Manager module is to delay loading by mgr_module_load_delay milliseconds. For testing purposes only.
...
**作用：** Period in seconds for collecting Manager modules cpu and memory performance counters. Period in seconds for Manager Monitor to collect the cpu and memory for each enabled module. If set to 0, collection of these stats will be disabled.
...
**作用：** Message queue depth that triggers automatic increase of mgr_stats_period When mgr_stats_period_autotune is enabled, the Manager will increase the stats reporting period if the incoming message queue exceeds this threshold. Higher values make the system less sensitive to temporary queue spikes but may allow longer periods of Manager overload.
...
**作用：** Lowest perfcounter priority collected by mgr Daemons only set perf counter data to the manager daemon if the counter has a priority higher than this.
...
**作用：** List of manager modules to load in independent subinterpreters A comma delimited list of module names. This list is read by manager when it starts. By default, manager loads each module into the main interpreter. Modules in this list will instead be loaded into independent subinterpreters. Specifying '*' will cause all modules to be run in independent subinterpreters.
